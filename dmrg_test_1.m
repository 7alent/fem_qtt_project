...
[U, lambda, t] = qtt_eig(A, B, 753, tol^2, tol);
...
U(:, 1) = U(:, 1)/sqrt(U(2:N, 1)'*B*U(2:N, 1));
U(:, 2) = U(:, 2)/sqrt(U(2:N, 2)'*B*U(2:N, 2));
...
title(['Plot of $u_h(x)$ when $q(x) = 10000cos(2^8x), N = 2^{', num2str(l), '}+1$'], 'FontSize', 24, 'Interpreter', 'latex')
